...
class Planet:
    ...
    def update_position(self,planets):
        ...
        for planet in planets:
            ...
            fx , fy = self.calculateForce(planet)
            total_fx +=fx
            total_fy += fy
        ...
        if(self.name == "mars"):
            if len(self.orbit) > 1200:
                self.orbit.pop(0)

        elif(self.name == "earth"):
            if len(self.orbit) > 600:
                self.orbit.pop(0)
...
def main():
    ...
    mercury = Planet(-0.387*Planet.AU, 0,8,GREY,3.30*10**23,"mercury")
